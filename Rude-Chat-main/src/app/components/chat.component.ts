  ...
  template: `
    <div class="chat-container">
      ...
      <mat-sidenav-container class="sidenav-container">        <mat-sidenav #sidenav mode="side" opened class="sidebar">
        ...
        <!-- Main Chat Area -->
        <mat-sidenav-content class="main-content">          <div class="chat-header">
            <button mat-icon-button (click)="sidenav.toggle()">
              <mat-icon>menu</mat-icon>
            </button>
            <h2>{{ chatService.currentSession()?.title || 'Rude Chat' }}</h2>
                ...
                <div class="message-avatar">
                  ...
                  <div class="message-text" [innerHTML]="formatMessageContent(message.content)"></div>                  @if (hasMapUrl(message.content)) {
                    <div class="map-container">
                      @if (getMapImageBlob(message.content)) {
                        <img 
...
export class ChatComponent implements OnInit, AfterViewChecked {
  ...
  hasMapUrl(content: string): boolean {
    return (content.includes('**Map URL:**') && 
           content.includes('atlas.microsoft.com/map/static/png')) ||
           (content.includes('![') && content.includes('atlas.azure.us/map/static/png'));
  }

  extractMapUrl(content: string): string | null {
    // First try the old format
    const mapUrlRegex = /\*\*Map URL:\*\*\s*(https:\/\/atlas\.microsoft\.com\/[^\s\n]+)/;
    const match = content.match(mapUrlRegex);
    if (match) {
      return match[1];
    }

    // Try markdown image format for Azure Government
    const markdownImageRegex = /!\[.*?\]\((https:\/\/atlas\.azure\.us\/[^)]+)\)/;
  ...
  formatMessageContent(content: string): string {
    ...
    let formatted = content
      ...
      // Line breaks
      .replace(/\n/g, '<br>')
      // Hide map URLs since we display them as images (both formats)
      .replace(/\*\*Map URL:\*\*[^\n]+/g, '')
      .replace(/!\[.*?\]\(https:\/\/atlas\.azure\.us\/[^)]+\)/g, '')
      // Clean up multiple consecutive line breaks
      .replace(/(<br>\s*){3,}/g, '<br><br>');    return formatted;
  }
  ...
  private async fetchMapImage(mapUrl: string): Promise<void> {
    try {
      console.log('Fetching map image:', mapUrl);
      
      // Fetch the image as blob
      const response = await this.http.get(mapUrl, { 
        responseType: 'blob',
        headers: {
      ...
      if (response) {        // Create blob URL
        const blobUrl = URL.createObjectURL(response);
        this.mapImageCache.set(mapUrl, blobUrl);
        console.log('Map image loaded successfully:', blobUrl);
  ...
  private async loadMoreSessions(): Promise<void> {    try {
    ...
    try {
      ...
      const response = await this.http.get(
        `${environment.api.baseUrl}/document/${doc.documentId}/download?userId=${encodeURIComponent(userId)}`,
        {
          responseType: 'blob'
        }
      ).toPromise();
