...
export class AgentActivityService {
  ...
  private handleRealtimeAgentActivity(data: any) {
    ...
    // Special handling for Multi-Agent System completion
    if (activity.agentName === 'Multi-Agent System' && activity.status === 'completed') {
      // Complete all remaining in-progress activities when the multi-agent system finishes
      this._activities.update(activities => {
        const updatedActivities = activities.map(act => {
          if (act.status === 'in-progress' || act.status === 'starting') {
            return {
              ...act,
              status: 'completed' as const,
              details: act.details + ' (Auto-completed when conversation finished)'
            };
          }
          return act;
        });
        
        // Add the multi-agent system completion activity
        return [...updatedActivities.slice(-99), activity];
      });
      
      // Reset all agent statuses to idle when multi-agent system completes
      ...
      console.log('✅ Multi-Agent System completed - auto-completed all pending activities');
      return;
    }
