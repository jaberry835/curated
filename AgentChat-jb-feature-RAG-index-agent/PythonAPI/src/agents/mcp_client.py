...
try:
    ...
    from src.tools.fictional_companies_tools import (
        get_ip_company_info_impl, get_company_devices_impl, get_company_summary_impl,
        health_check_impl as fictional_health_check_impl
    )
    from src.tools.rag_dataset_tools import (
        search_hulk_dataset_impl, search_policy_documents_dataset_impl,
        get_rag_dataset_tools, get_rag_dataset_tool_descriptions
    )
...
class MCPClient:
    ...
    def __init__(self, server_url: str = "http://localhost:3001", user_id: str = None, session_id: str = None, adx_token: str = None):
        ...
        Args:
            ...
            adx_token: ADX access token for user impersonation
        """
        ...
        self.tool_functions = {
            ...
            "get_document": get_document_metadata_impl,  # Map get_document to get_document_metadata_impl
            ...
            # Fictional companies tools
            "get_ip_company_info": get_ip_company_info_impl,
            "get_company_devices": get_company_devices_impl,
            "get_company_summary": get_company_summary_impl,
            "fictional_health_check": fictional_health_check_impl,
            # RAG dataset tools
            "search_hulk_dataset": search_hulk_dataset_impl,
            "search_policy_documents_dataset": search_policy_documents_dataset_impl,
            # Additional document tools that might be implemented in the future
            "delete_document": None,
            "process_document": None
        }
        
        # Add dynamic RAG dataset tools
        try:
            rag_tools = get_rag_dataset_tools()
            for tool_name, tool_func in rag_tools.items():
                self.tool_functions[tool_name] = tool_func
        except Exception as e:
            print(f"⚠️ Failed to load dynamic RAG dataset tools: {e}")
    ...
    async def _fetch_tools(self):
        ...
        self.tools = [
            ...
            # Document tools
            {"name": "list_documents", "description": "List all available documents in the document store"},
            {"name": "get_document_metadata", "description": "Get metadata for a specific document"},
            {"name": "download_document", "description": "Download a document from the document store"},
            {"name": "search_documents", "description": "Search for documents using natural language queries"},
            {"name": "get_document_content_summary", "description": "Get a content summary of a specific document"},
            # Fictional companies tools
            {"name": "get_ip_company_info", "description": "Get fictional company information for a given IP address"},
            {"name": "get_company_devices", "description": "Get fictional device information for a given company"},
            {"name": "get_company_summary", "description": "Get comprehensive fictional summary for a given company"},
            {"name": "fictional_health_check", "description": "Check if the fictional companies API is healthy and available"},
            # RAG dataset tools
            {"name": "search_hulk_dataset", "description": "Search the Hulk Dataset for Hulk-related information"},
            {"name": "search_policy_documents_dataset", "description": "Search the Policy Documents dataset for corporate policies and procedures"}
        ]
        
        # Add dynamic RAG dataset tool descriptions
        try:
            rag_tool_descriptions = get_rag_dataset_tool_descriptions()
            for desc in rag_tool_descriptions:
                if desc["name"] not in [tool["name"] for tool in self.tools]:
                    self.tools.append({
                        "name": desc["name"],
                        "description": desc["description"]
                    })
        except Exception as e:
            print(f"⚠️ Failed to load dynamic RAG dataset tool descriptions: {e}")
    ...
    async def call_tool(self, tool_name: str, arguments: dict) -> str:
        ...
        try:
            ...
            # Fictional companies tools for emitting events
            fictional_companies_tools = ["get_ip_company_info", "get_company_devices", 
                                       "get_company_summary", "fictional_health_check"]
            
            # RAG dataset tools for emitting events
            rag_dataset_tools = ["search_hulk_dataset", "search_policy_documents_dataset"]
            
            # Add dynamic RAG dataset tools to the list
            try:
                rag_tool_descriptions = get_rag_dataset_tool_descriptions()
                for desc in rag_tool_descriptions:
                    if desc["name"] not in rag_dataset_tools:
                        rag_dataset_tools.append(desc["name"])
            except Exception as e:
                logger.warning(f"Failed to load dynamic RAG dataset tools for SSE: {e}")
            ...
            # ADX tools specific events and session_id/adx_token injection
                ...
                # Inject adx_token if not already provided in arguments
                if hasattr(self, 'adx_token') and self.adx_token and 'adx_token' not in arguments:
                    arguments['adx_token'] = self.adx_token
                    logger.info(f"🔑 Injecting adx_token for user impersonation for tool: {tool_name}")
                elif not hasattr(self, 'adx_token') or not self.adx_token:
                    logger.info(f"🔑 No adx_token available, using system identity for tool: {tool_name}")
            ...
            # Fictional companies tools specific events and session_id injection
            elif tool_name in fictional_companies_tools:
                # Inject session_id if not already provided in arguments
                if hasattr(self, 'session_id') and self.session_id and 'session_id' not in arguments:
                    arguments['session_id'] = self.session_id
                    logger.info(f"🔑 Injecting session_id: {self.session_id} for tool: {tool_name}")
                ...
                if hasattr(self, 'session_id') and self.session_id and has_app_context():
                    try:
                        action_desc = "Using fictional companies API"
                        details = f"Running {tool_name}"
                        
                        if tool_name == "get_ip_company_info":
                            ip_address = arguments.get("ip_address", "")
                            details = f"Getting company info for IP: {ip_address}"
                        elif tool_name == "get_company_devices":
                            company_name = arguments.get("company_name", "")
                            details = f"Getting devices for company: {company_name}"
                        elif tool_name == "get_company_summary":
                            company_name = arguments.get("company_name", "")
                            details = f"Getting summary for company: {company_name}"
                        elif tool_name == "fictional_health_check":
                            details = "Checking fictional companies API health"
                            
                        agent_name = "Fictional Companies Agent"
                        sse_emitter.emit_agent_activity(
                            session_id=self.session_id,
                            agent_name=agent_name,
                            action=action_desc,
                        ...
                        logger.warning(f"Failed to emit fictional companies tool activity: {str(emit_error)}")
            
            # RAG dataset tools specific events and session_id injection
            elif tool_name in rag_dataset_tools:
                # Inject session_id if not already provided in arguments
                if hasattr(self, 'session_id') and self.session_id and 'session_id' not in arguments:
                    arguments['session_id'] = self.session_id
                    logger.info(f"🔑 Injecting session_id: {self.session_id} for tool: {tool_name}")
                
                # Inject user_id if not already provided in arguments
                if hasattr(self, 'user_id') and self.user_id and 'user_id' not in arguments:
                    arguments['user_id'] = self.user_id
                    logger.info(f"🔑 Injecting user_id: {self.user_id} for tool: {tool_name}")
                ...
                if hasattr(self, 'session_id') and self.session_id and has_app_context():
                    try:
                        action_desc = "Searching dataset"
                        details = f"Using RAG tool: {tool_name}"
                        
                        # Extract dataset name from tool name
                        dataset_name = tool_name.replace("search_", "").replace("_dataset", "")
                        
                        if "hulk" in tool_name:
                            action_desc = "Searching Hulk dataset"
                            query = arguments.get("query", "")
                            details = f"Searching Hulk dataset for: {query}"
                        elif "policy" in tool_name:
                            action_desc = "Searching policy documents"
                            query = arguments.get("query", "")
                            details = f"Searching policy documents for: {query}"
                        else:
                            query = arguments.get("query", "")
                            details = f"Searching {dataset_name} dataset for: {query}"
                        
                        agent_name = f"{dataset_name.title()} RAG Agent"
                        sse_emitter.emit_agent_activity(
                            session_id=self.session_id,
                            agent_name=agent_name,
                            action=action_desc,
                        ...
                        logger.warning(f"Failed to emit RAG dataset tool activity: {str(emit_error)}")
            ...
            # Handle fictional companies tool results
            if tool_name in fictional_companies_tools and hasattr(self, 'session_id') and self.session_id and has_app_context():
                try:
                    result_obj = json.loads(result) if isinstance(result, str) else result
                    status = "completed"
                    details = "Fictional companies operation completed"
                    
                    if isinstance(result_obj, dict):
                        success = result_obj.get("status") == "success"
                        status = "completed" if success else "error"
                        
                        if tool_name == "get_ip_company_info":
                            ip_address = arguments.get("ip_address", "")
                            if success:
                                company_name = result_obj.get("data", {}).get("company", {}).get("name", "Unknown")
                                details = f"Found fictional company '{company_name}' for IP {ip_address}"
                            else:
                                details = f"Failed to get company info for IP {ip_address}"
                        elif tool_name == "get_company_devices":
                            company_name = arguments.get("company_name", "")
                            if success:
                                device_count = result_obj.get("data", {}).get("total_devices", 0)
                                details = f"Found {device_count} fictional devices for company {company_name}"
                            else:
                                details = f"Failed to get devices for company {company_name}"
                        elif tool_name == "get_company_summary":
                            company_name = arguments.get("company_name", "")
                            if success:
                                details = f"Retrieved fictional company summary for {company_name}"
                            else:
                                details = f"Failed to get summary for company {company_name}"
                        elif tool_name == "fictional_health_check":
                            if success:
                                details = "Fictional companies API is healthy"
                            else:
                                details = "Fictional companies API health check failed"
                    
                    sse_emitter.emit_agent_activity(
                        session_id=self.session_id,
                        agent_name="Fictional Companies Agent",
                        action="Fictional companies operation results",
                        status=status,
                        details=details
                    )
                except Exception as emit_error:
            ...
            # Handle RAG dataset tool results
            if tool_name in rag_dataset_tools and hasattr(self, 'session_id') and self.session_id and has_app_context():
                try:
                    result_obj = json.loads(result) if isinstance(result, str) else result
                    status = "completed"
                    details = "RAG dataset search completed"
                    
                    if isinstance(result_obj, dict):
                        success = result_obj.get("success", False)
                        result_count = result_obj.get("count", 0)
                        dataset_name = result_obj.get("dataset", "unknown")
                        
                        status = "completed" if success else "error"
                        
                        if success and result_count > 0:
                            details = f"Found {result_count} relevant documents in {dataset_name} dataset"
                            # Add file names if results exist
                            ...
                            status = "no-results"
                            details = f"No relevant documents found in {dataset_name} dataset"
                        else:
                            error_msg = result_obj.get("error", "Unknown error")
                            details = f"RAG dataset search error: {error_msg}"
                        
                        # Extract dataset name for agent name
                        display_dataset_name = dataset_name.replace("_", " ").title()
                        agent_name = f"{display_dataset_name} RAG Agent"
                        
                        sse_emitter.emit_agent_activity(
                            session_id=self.session_id,
                            agent_name=agent_name,
                            action="RAG dataset search results",
                            status=status,
                            details=details
                        )
                except Exception as emit_error:
                    logger.warning(f"Failed to emit RAG dataset tool results activity: {str(emit_error)}")
    ...
    def get_tools_by_category(self, category: str) -> List:
        ...
        elif category == "fictional_companies":
            fictional_companies_tools = ["get_ip_company_info", "get_company_devices", 
                                       "get_company_summary", "fictional_health_check"]
            return [tool for tool in self.tools if tool["name"] in fictional_companies_tools]
        elif category == "rag_datasets":
            # Get all RAG dataset tools dynamically
            rag_tools = [tool for tool in self.tools if tool["name"].startswith("search_") and tool["name"].endswith("_dataset")]
            return rag_tools
        elif category == "adx":
            adx_tools = ["adx_list_databases", "adx_list_tables", "adx_describe_table", 
                        "adx_execute_query", "adx_get_cluster_info"]
            return [tool for tool in self.tools if tool["name"] in adx_tools]
        elif category == "documents":
            document_tools = ["list_documents", "get_document_metadata", "download_document", 
                            "search_documents", "get_document_content_summary"]
            return [tool for tool in self.tools if tool["name"] in document_tools]
        else:
            return self.tools
