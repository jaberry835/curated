...
async def build_agent() -> ChatCompletionAgent:
    ...
    agent = ChatCompletionAgent(
        ...
        name="ADXAgent",
        instructions=(
            "You are an Azure Data Explorer (ADX) specialist agent. You ONLY respond to ADX/database questions. KQL is the query language, do not try to query using SQL!\n\n"
            ...
            "4. ONLY THEN: kusto_query(database_name, query) - with the correct names using Kusto (KQL) not SQL or another language\n\n"
            ...
            "Step 4: kusto_query('SecurityDB', 'scans | where ip_address == \"103.13.92.73\"')\n\n"
            "🔧 **ERROR RECOVERY:**\n"
            ...
            "- kusto_query(database_name, query) - Use exact names, ONLY after discovery and only use KQL syntax, never SQL\n"
            "- kusto_get_cluster_info() - For cluster information\n\n"
            "⚡ **CRITICAL RULES:**\n"
            "1. ALWAYS start with kusto_list_databases() for ANY task\n"
            "2. NEVER use placeholder names like 'your_database_name'\n"
            "3. NEVER assume database or table names exist\n"
            "4. Use exact names returned by your tools\n"
            "5. If user mentions 'scans table' → find which database has table named 'scans'\n\n"
            "Remember: Discovery first, query second. Always!"
        ),
...
def well_known_agent_card():
    ...
    return jsonify({
        "name": "ADXAgent",
        "description": "Azure Data Explorer (KQL) specialist backed by MCP ADXTools.",
