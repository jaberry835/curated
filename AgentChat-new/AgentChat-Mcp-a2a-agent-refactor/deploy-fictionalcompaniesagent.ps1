...
function Get-DeploymentConfig {
    Write-Info "Setting up FictionalCompaniesAgent deployment configuration..."
    
    if ([string]::IsNullOrEmpty($AppName)) {
        $AppName = Read-Host "Enter your FictionalCompaniesAgent App Service name (e.g., myapp-FictionalCompaniesAgent)"
    }
    ...
    Write-Host ""
    Write-Info "FictionalCompaniesAgent Deployment Configuration:"
    Write-Host "  App Name: $AppName"
    Write-Host "  Resource Group: $ResourceGroup"
    Write-Host "  Location: $Location"
    Write-Host "  Python Version: $PythonVersion"
    Write-Host "  Startup Command: gunicorn src.remote_agents.fictional_companies_agent_wsgi:application"
    Write-Host ""
    Write-Host ""
...
function Set-EnvironmentVariables {
    ...
    # Set the startup command to use gunicorn with correct module path for FictionalCompanies Agent
    az webapp config set `
        --name $Config.AppName `
        --resource-group $Config.ResourceGroup `
        --startup-file "gunicorn --bind 0.0.0.0:8000 --workers 1 --threads 4 --timeout 300 --keep-alive 75 --no-sendfile --max-requests 1000 --max-requests-jitter 50 --access-logfile - --error-logfile - src.remote_agents.fictional_companies_agent_wsgi:application"
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        az webapp config appsettings set `
            ...
            --resource-group $Config.ResourceGroup `
            --settings `
            STARTUP_COMMAND="gunicorn --bind 0.0.0.0:8000 --workers 1 --threads 4 --timeout 300 --keep-alive 75 --no-sendfile --max-requests 1000 --max-requests-jitter 50 --access-logfile - --error-logfile - src.remote_agents.fictional_companies_agent_wsgi:application"
    }
    ...
    # Force Python app detection and dependency installation for ADX Agent
    az webapp config appsettings set `
        --name $Config.AppName `
        --resource-group $Config.ResourceGroup `
        --settings `
...
function Deploy-Application {
    ...
    try {
        ...
        if (!(Test-Path "PythonAPI\src\remote_agents\fictional_companies_agent_wsgi.py")) {
            Write-Error "fictional_companies_agent_wsgi.py not found in PythonAPI directory" 
            exit 1
        }
        
        Write-Info "Including main.py, fictional_companies_agent_wsgi.py, and requirements.txt in deployment..."
        ...
        $requiredFiles = @("requirements.txt", "main.py", "src/remote_agents/fictional_companies_agent_wsgi.py")
        $missingFiles = @()
...
function Show-DeploymentInfo {
    ...
    Write-Info "FictionalCompaniesAgent deployment completed successfully!"
    Write-Host ""
    Write-Info "FictionalCompaniesAgent Information:"
...
function Main {
    Write-Host "========================================================"
    Write-Host "FictionalCompaniesAgent Deployment Script"
    Write-Host "Fictional Companies Agent Service"
    Write-Host "========================================================"
    Write-Host ""
