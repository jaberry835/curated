...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MarkdownModule, AngularSplitModule],
...
export class ChatWithDataComponent {
  ...
  onTabSelect(tab: string): void {
    ...
    // Perform actions based on which tab was selected
    switch (tab) {
      case 'docSummary':
        
        // Add logic to build or load the Doc Summary content
        break;
      case 'aoaiTranslate':
       
        // Add logic to trigger AOAI translation
        break;
      case 'azureTranslate':
       
        // Add logic to trigger Azure translation
        break;
      default:
      
        // Add logic for the default preview tab if needed
        break;
    }
  }
  ...
  openDocument() {
    const docUrl = encodeURIComponent('https://example.com/path/to/document.docx');
    window.open(`/doc-viewer?docUrl=${docUrl}`, '_blank');
  }

}
function forRoot(): readonly any[] | import("@angular/core").Type<any> {
