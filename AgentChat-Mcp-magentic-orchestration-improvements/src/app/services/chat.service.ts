...
export class ChatService {  private http = inject(HttpClient);
  ...
  private agentActivityService = inject(AgentActivityService);// Signals for reactive state management
  ...
  constructor() {
    ...
    this.initializeMCP(); // Re-enabled for MCP agent testing
    // console.log('MCP initialization enabled for agent testing');
  }
  ...
  async createNewSession(): Promise<ChatSession> {
    ...
    try {      // Create session on the backend
      ...
      // Join agent activity session for real-time updates
      await this.agentActivityService.joinSession(newSession.id);
    ...
    try {      const params: any = {
      ...
      if (session) {
        ...
        // Load memory context for the session (on reset/initial load)
        if (reset) {
          try {
            const memoryResult = await this.loadSessionMemory(sessionId);
            if (memoryResult.success && memoryResult.contextSummary) {
              console.log('📚 Session memory loaded with context:', memoryResult.contextSummary.substring(0, 100) + '...');
            }
          } catch (memoryError) {
            console.warn('Memory loading failed, continuing without memory context:', memoryError);
          }
          
          // Leave previous session first
          await this.agentActivityService.leaveSession();
    ...
    this.isTyping.set(true);    try {      // Save user message to backend (async, non-blocking for UI)
      ...
      try {
        ...
        if (searchResult.results && searchResult.results.length > 0) {
          ...
          // 🔧 TOKEN MANAGEMENT: Limit RAG context to prevent token overflow
          const maxRagTokens = 4000; // Reserve ~4K tokens for RAG context
          let ragContext = '';
          let usedTokens = 0;
          
          // Sort by relevance and include top results within token limit
          const sortedResults = searchResult.results
            .sort((a, b) => (b.score || 0) - (a.score || 0))
            .slice(0, 5); // Limit to top 5 results
          
          for (const result of sortedResults) {
            const entryContent = `Document: ${result.documentId}\nContent: ${result.content}`;
            const estimatedTokens = Math.ceil(entryContent.length / 4); // Rough token estimate
            
            if (usedTokens + estimatedTokens <= maxRagTokens) {
              ragContext += `${entryContent}\n\n`;
              usedTokens += estimatedTokens;
            } else {
              // Try to fit a truncated version
              const remainingChars = (maxRagTokens - usedTokens) * 3; // Conservative character estimate
              if (remainingChars > 100) {
                const truncatedContent = result.content.substring(0, remainingChars - 50) + '...[truncated]';
                ragContext += `Document: ${result.documentId}\nContent: ${truncatedContent}\n\n`;
              }
              break;
            }
          }
          
          console.log(`📄 RAG context optimized: ~${usedTokens} tokens from ${searchResult.results.length} results`);
        } else {
      ...
      // Process agent interactions if present
      if (response.agentInteractions && response.agentInteractions.length > 0) {
        console.log('Processing agent interactions:', response.agentInteractions);
        
        // Convert backend AgentInteraction to frontend AgentActivity
        const activities = response.agentInteractions.map(interaction => ({
          id: interaction.id,
          agentId: this.mapAgentNameToId(interaction.agentName),
          agentName: interaction.agentName,
          action: interaction.action,
          status: this.mapStatusToActivity(interaction.status),
          timestamp: new Date(interaction.timestamp),
          details: interaction.result,
          duration: interaction.duration ? interaction.duration / 1000 : undefined // Convert from ms to seconds
        }));
        
        // Add activities to the agent activity service
        this.agentActivityService.addActivities(activities);
      }
      ...
      // Save conversation memory after successful completion
      try {
        await this.saveSessionMemory(currentSession.id);
        console.log('💾 Session memory saved after successful conversation');
      } catch (memoryError) {
        console.warn('Failed to save session memory, conversation will continue:', memoryError);
      }
  ...
  async loadSessionMemory(sessionId: string): Promise<{ success: boolean; contextSummary?: string }> {
    const userId = this.authService.getUserId();
    if (!userId) throw new Error('User not authenticated');

    try {
      const response = await firstValueFrom(
        this.http.post<{ success: boolean; contextSummary: string }>(`${this.baseUrl}/session/${sessionId}/memory/load`, {
          userId
        })
      );
      
      console.log(`Memory loaded for session ${sessionId}:`, response.contextSummary?.substring(0, 100) + '...');
      return response;
    } catch (error) {
  ...
  async saveSessionMemory(sessionId: string): Promise<{ success: boolean }> {
    const userId = this.authService.getUserId();
    if (!userId) throw new Error('User not authenticated');

    try {
      const response = await firstValueFrom(
        this.http.post<{ success: boolean }>(`${this.baseUrl}/session/${sessionId}/memory/save`, {
          userId
        })
      );
  ...
  async reduceSessionMemory(sessionId: string, targetCount: number = 30): Promise<{ success: boolean; wasReduced: boolean }> {
    const userId = this.authService.getUserId();
    if (!userId) throw new Error('User not authenticated');

    try {
      const response = await firstValueFrom(
        this.http.post<{ success: boolean; wasReduced: boolean }>(`${this.baseUrl}/session/${sessionId}/memory/reduce`, {
          userId,
          targetCount
        })
      );
      
      console.log(`Memory reduction for session ${sessionId}: ${response.wasReduced ? 'performed' : 'not needed'}`);
      return response;
    } catch (error) {
      console.error('Error reducing session memory:', error);
      throw error;
    }
  }

  async getSessionContext(sessionId: string, maxChars: number = 1000): Promise<{ sessionId: string; contextSummary: string; maxChars: number }> {
    try {
      const response = await firstValueFrom(
        this.http.get<{ sessionId: string; contextSummary: string; maxChars: number }>(`${this.baseUrl}/session/${sessionId}/context?maxChars=${maxChars}`)
      );
      
      console.log(`Context retrieved for session ${sessionId}:`, response.contextSummary.substring(0, 100) + '...');
      return response;
    } catch (error) {
  ...
  async getSessionTokenStats(sessionId: string): Promise<any> {
    try {
      const response = await firstValueFrom(
        this.http.get<any>(`${this.baseUrl}/session/${sessionId}/tokens`)
      );
      
      console.log(`Token stats for session ${sessionId}:`, response);
      return response;
    } catch (error) {
      console.error('Error getting session token stats:', error);
      throw error;
    }
  }

  async optimizeSessionTokens(sessionId: string, maxTokens?: number): Promise<any> {
    try {
      const response = await firstValueFrom(
        this.http.post<any>(`${this.baseUrl}/session/${sessionId}/tokens/optimize`, {
          maxTokens
        })
      );
      
      console.log(`Token optimization for session ${sessionId}:`, response);
      return response;
    } catch (error) {
      console.error('Error optimizing session tokens:', error);
      throw error;
    }
  }

  // Enhanced session loading with memory support
  // Pagination getters
  ...
  private mapAgentNameToId(agentName: string): string {
    const agentMap: { [key: string]: string } = {
      'Orchestrator': 'core-agent',
      'ADX Agent': 'adx-agent',
      'Maps Agent': 'maps-agent',
      'Documents Agent': 'documents-agent',
      'Azure Agent': 'resources-agent'
    };
    return agentMap[agentName] || 'core-agent';
  }

  private mapStatusToActivity(status: string): 'starting' | 'in-progress' | 'completed' | 'error' {
    const statusMap: { [key: string]: 'starting' | 'in-progress' | 'completed' | 'error' } = {
