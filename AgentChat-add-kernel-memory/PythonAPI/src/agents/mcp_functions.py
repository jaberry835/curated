...
class MCPFunctionWrapper:
    ...
    def create_fictional_companies_functions(self):
        """Create Semantic Kernel functions for fictional companies tools."""
        
        @kernel_function(name="get_ip_company_info", description="Get fictional company information for a given IP address")
        async def get_ip_company_info(
            ip_address: Annotated[str, "The IP address to lookup company information for"]
        ) -> Annotated[str, "Fictional company information in JSON format"]:
            return await self.mcp_client.call_tool("get_ip_company_info", {"ip_address": ip_address})
        
        @kernel_function(name="get_company_devices", description="Get fictional device information for a given company")
        async def get_company_devices(
            company_name: Annotated[str, "The name of the company to get device information for"]
        ) -> Annotated[str, "Fictional device information in JSON format"]:
            return await self.mcp_client.call_tool("get_company_devices", {"company_name": company_name})
        
        @kernel_function(name="get_company_summary", description="Get comprehensive fictional summary for a given company")
        async def get_company_summary(
            company_name: Annotated[str, "The name of the company to get summary information for"]
        ) -> Annotated[str, "Fictional company summary in JSON format"]:
            return await self.mcp_client.call_tool("get_company_summary", {"company_name": company_name})
        
        @kernel_function(name="fictional_health_check", description="Check if the fictional companies API is healthy and available")
        async def fictional_health_check() -> Annotated[str, "Health check result in JSON format"]:
            return await self.mcp_client.call_tool("fictional_health_check", {})
        
        return [get_ip_company_info, get_company_devices, get_company_summary, fictional_health_check]
